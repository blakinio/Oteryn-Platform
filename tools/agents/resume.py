...
def build_bundle(path: Path) -> dict[str, object]:
    ...
    errors = checkpoint.validate_checkpoint(parsed.data, contract, source=path)
    if errors:
    ...
    return {"task_id": task_id(path), "checkpoint": str(path), **{key: data.get(key) for key in keys}}
...
def render(data: dict[str, object]) -> str:
    ...
    if isinstance(failure, dict):
        lines += [f"FIRST_FAILURE_MARKER: {failure.get('marker', 'none')}", f"FIRST_FAILURE_EVIDENCE: {failure.get('evidence', 'none')}"]
    lines.append("VALIDATION:")
    for item in data.get("validation", []):
        if isinstance(item, dict):
            lines.append(f"- {item.get('command', '')}: {item.get('result', '')}; evidence={item.get('evidence', '')}")
    lines += [
        f"NEXT_ACTION: {data.get('next_action', 'UNKNOWN')}", "", "OPERATING_RULES:",
